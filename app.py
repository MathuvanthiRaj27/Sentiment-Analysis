...
df = pd.read_excel("feedback-details-report-rio_f3800e23e4bbf1f0d925a619aed9c0c8-1227318.xlsx",sheet_name="Feedback Details")
...
@app.route('/statetable', methods=['POST'])
def statetable():
    state=request.form['state']
    if state:
        res_df = df[df['State'] == state.upper()]
        res_df.drop(['Original Feedback Language (if not English)', 'reviews', 'Engagement Response', 'URL', 'Source'],axis=1, inplace=True)

    return render_template("file.html", column_names=res_df.columns.values, result=list(res_df.values.tolist()), zip=zip)

@app.route('/citytable', methods=['POST'])
def citytable():
    city = request.form['city']
    if city:
        res_df = df[df['City'].str.upper() == city.upper()]
        res_df.drop(['Original Feedback Language (if not English)', 'reviews', 'Engagement Response', 'URL', 'Source'],axis=1, inplace=True)

    return render_template("file.html", column_names=res_df.columns.values, result=list(res_df.values.tolist()), zip=zip)

@app.route('/datetable', methods=['POST'])
def datetable():
    s = request.form.get("startvalue")
    e = request.form.get("endvalue")
    after_start_date = df["Review Date"] >= s
    before_end_date = df["Review Date"] <= e
    print(before_end_date)
    between_two_dates = after_start_date & before_end_date
    print(between_two_dates)
    res_df = df.loc[between_two_dates]
    return render_template("file.html", column_names=res_df.columns.values, result=list(res_df.values.tolist()), zip=zip)
...
def overview():
    ...
    writer.close()
    return render_template('overview.html', total=total_count, p_count=result[0], ne_count=result[1], n_count=result[2],
                           t_percent="Total percent is 100%", p_percent=pos_per,words =list(pos_freq['word'].values.tolist())[0:9],
                           pos_freq_count = list(pos_freq['count'].values.tolist())[0:9], ne_percent=neu_per, n_percent=neg_per,
                           words_neg=list(neg_freq['word'].values.tolist())[0:9],neg_freq_count = list(neg_freq['count'].values.tolist())[0:9],
...
def state():
    state = request.form['input-small']
